...
class kernal(object):
    def __init__(self, car_num, render=False, record=True):
        ...
        self.motion = 6
        self.rotate_motion = 0.1
        self.yaw_motion = 1
        self.camera_angle = 75 / 2
        self.lidar_angle = 120 / 2
        self.move_discount = 0.6 # 1.3 #change to adjust how to deal collision with the wall
        # above are params that can be challenged depended on situation
        #original w: 8000, l: 12000 x0.08
        self.map_length = 960
        self.map_width = 640
        ...
        #1: 2: 3:Start Area
        self.areas = np.array([[#red
                                [480, 150, 270, 370.0], #mid area
                                [50.0, 50.0, 0.0, 100.0], #supply area
                                # [700.0, 800.0, 400.0, 500.0],
                                [50, 150.0,540, 640.0]], #start place
                               
                               [#blue
                                [480, 150, 270, 370.0],
                                [910, 910, 540, 640.0],
                                # [0.0, 100.0, 0.0, 100.0],
                                [910.0, 960.0, 0.0, 100.0]]], dtype='float32') #50, 150.0,540, 640.0
        
        self.barriers = np.array([#horizontal x-y 
                                [0, 100.0, 480, 505],
                                  [860.0, 960.0, 140.0, 165.0],
                                  #vertical                                  
                                  [820, 845.0, 0.0, 100.0],
                                  [150.0, 170.0, 540.0, 640.0],
                                  
                                  [570.0, 595.0, 220.0, 320.0],
                                  [365.0, 390.0, 320, 420.0]], dtype='float32')
        if render:
            global pygame
            import pygame
            pygame.init()
            ...
            self.barriers_rect = []
            for i in range(self.barriers.shape[0]):
                self.barriers_img.append(pygame.image.load('./imgs/barrier_{}.png'.format('horizontal' if i < 2 else 'vertical')))
                self.barriers_rect.append(self.barriers_img[-1].get_rect())
                self.barriers_rect[-1].center = [self.barriers[i][0:2].mean(), self.barriers[i][2:4].mean()]
            # load areas imgs
            self.areas_img = []
            ...
            for oi, o in enumerate(['red', 'blue']):
                for ti, t in enumerate(['bonus', 'supply', 'start']): #'start',
                    ...
                    self.areas_img.append(pygame.image.load('./imgs/area_{}_{}.png'.format(t, o)))
                    self.areas_rect.append(self.areas_img[-1].get_rect())
                    self.areas_rect[-1].center = [self.areas[oi, ti][0:1].mean(), self.areas[oi, ti][2:4].mean()]
    ...
    def reset(self):
        ...
        #initialize -- Car info
        cars = np.array([[1, 250, 400, 0, 0, 0, 2000, 0, 0, 1, 0, 0, 0, 0, 0],
                         [0, 750, 50, 0, 0, 0, 2000, 0, 0, 1, 0, 0, 0, 0, 0], # 50 ,450
    ...
    def move_car(self, n):
        ...
        if not self.cars[n, 7]:
            ...
            if self.acts[n, 2] or self.acts[n, 3]:
                ...
                if self.check_interface(n):
                    print("Back2")
                    self.acts[n, 2] = -self.acts[n, 2] * self.move_discount  #* 1.3
                    self.cars[n, 1] = p
                # y
                p = self.cars[n, 2]
                ...
                if self.check_interface(n):
                    print("Back3")
                    self.acts[n, 3] = -self.acts[n, 3] * self.move_discount  #* 1.3
                    self.cars[n, 2] = p
    ...
    def orders_to_acts(self, n):
        ...
        if self.acts[n, 3] >= 1: self.acts[n, 3] = 0.8
        if self.acts[n, 3] <= -1: self.acts[n, 3] = -0.8
        ...
        if self.acts[n, 0] > 1: self.acts[n, 0] = 0.5
        if self.acts[n, 0] < -1: self.acts[n, 0] = -0.5
